...
Module TorqueCalc
    ...
    Public MaxNumberRopes As Integer
    ...
    '    Dim dZ2, dY1, dX1, dZ1, dZ3 As Double
    ...
    '        If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE And ME_MRM01Car_typ.ControllerType <> DRIVETYPE_SCRDC And _
    '           ME_MRM01Car_typ.ControllerType <> DRIVETYPE_QUATTRO Then
    '            Exit Sub
    '        End If
    ...
    '            iIndex = 1
    '            sSQL = "SELECT * "
    '            sSQL &= "From [Drive Selection] "
    '            sSQL &= "WHERE [Drive Selection].ANSI="
    '            If ME_ADM01Bnk_typ.ANSICode >= 2000 Then
    '                sSQL &= "True"
    '            Else
    '                sSQL &= "False"
    '            End If
    ...
    '            WorkingRecordSet.MoveFirst()
    '            Do Until WorkingRecordSet.EOF
    '                ReDim Preserve DriveListByCost(iIndex)
    '                DriveListByCost(iIndex).DriveSelection = WorkingRecordSet("Drive Selection").Value.ToString
    '                DriveListByCost(iIndex).IRFLU = WorkingRecordSet("IRFLU").Value.ToString
    '                DriveListByCost(iIndex).IAFLU = WorkingRecordSet("IAFLU").Value.ToString
    '                iIndex += 1
    ...
    '        End If
    '        Dependents.Set_RopeStrength()
    '        bOK = False
    '        ClearErrors()
    '        VFLU = CStr(ME_MRM01Car_typ.NamePlateArmatureFullLoadVoltage)
    '        VFLU = CStr(Conversion.Val(VFLU))
    ...
    '            If (ME_PIT01Car_typ.Compensation = "Reuse" Or ME_PIT01Car_typ.Compensation = "Relocate") And ME_PIT01Car_typ.CableChainSize = "Unknown" Then
    '                If ME_MRM01Car_typ.RopingNew = "1:1" Then
    '                    iRoping = 1
    '                Else
    '                    iRoping = 2
    '                End If
    '                dRWT = Conversion.Val(CStr(ME_MRM01Car_typ.HoistCableQty)) * Conversion.Val(MN_TRQ01_typ.RopeWeight)
    '                dIdealCompWeight = iRoping * dRWT - dCtrlCableWt / 4
    '                dCompWt = dIdealCompWeight / 2
    '            Else
    '                dCompWt = ME_PIT01Car_typ.CableChainQty * Get_CompUnitWeight()
    '            End If
    '        Else
    '            dCompWt = Conversion.Val(MN_TRQ01_typ.CompensationWeight)
    '        End If
    '        dCWT = dCarWeight + (dCounterWeight / 100) * dCapacity + dCtrlCableWt * (dRise + dXtraCCable) / 4
    '        If ME_MRM01Car_typ.RopingNew = "1:1" Or ME_CAR01Car_typ.TopOfCarSheave = "None" Then
    '            iRoping = 1
    '            dCSH = 0
    '            dCSH1 = 0
    '            dCwtSH = 0
    ...
    '            dIdealCompWeight = iRoping * dRWT - dCtrlCableWt / 4
    '            If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '                If Conversion.Val(MN_TRQ01_typ.NominalMotorHP) = 0 Or bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    '                    MotorInertiasNew(False)
    '                Else
    '                    dMH = Conversion.Val(MN_TRQ01_typ.NominalMotorHP)
    '                    MN_TRQ01_typ.MotorRPM = Conversion.Val(Conversion.Val(ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False)))
    ...
    '                        gTRFLU = WorkingRecordSet("TRFLU").Value
    '                        gTAFLU = WorkingRecordSet("TAFLU").Value
    '                        dMotorInertia = WorkingRecordSet("Motor Inertia").Value
    '                        dATork = gTAFLU
    ...
    '                           ((ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR And Strings.Right(DriveListByCost(iIndex).DriveSelection, 2) = "BR") Or
    '                           (ME_MRM01Car_typ.ControllerType = DRIVETYPE_PF1 And Strings.Right(DriveListByCost(iIndex).DriveSelection, 3) = "PF1")) Then
    ...
    '        dCapacity = Conversion.Val(CStr(0) & CStr(ME_COM01Car_typ.Capacity))
    '        dCtrlCableWt = Conversion.Val(("" & MN_TRQ01_typ.ControlCableWeight).Trim())
    '        dRise = Math.Round(Conversion.Val(Conversion.Val(CStr(ME_COM01Car_typ.TravelFt)) + Conversion.Val(CStr(ME_COM01Car_typ.TravelIn / 12))), 2)
    '        dXtraCCable = Conversion.Val(CStr(0) & MN_TRQ01_typ.len3)
    '        dMsDia = Conversion.Val(CStr(0) & ME_MRM01Car_typ.MachineSheaveDia)
    '        dGR = Conversion.Val(CStr(0) & MN_TRQ01_typ.GearRation1)
    '        If Conversion.Val(MN_TRQ01_typ.Acceleration) = 0 Or Conversion.Val(MN_TRQ01_typ.Deceleration) = 0 Or Conversion.Val(MN_TRQ01_typ.DecelerationTraction) = 0 Or Conversion.Val(MN_TRQ01_typ.DecelerationD125) = 0 Or Conversion.Val(MN_TRQ01_typ.DecelerationTSD) = 0 Or Conversion.Val(MN_TRQ01_typ.TractionAdder) = 0 Then
    '            Calc_GeneralData()
    '        End If
    ...
    '        dInputSpeed = Conversion.Val(CStr(0) & CStr(ME_COM01Car_typ.Speed))
    ...
    '        sMachineType = ME_MRM01Car_typ.MachineModel
    '        dMH = Conversion.Val(MN_TRQ01_typ.NominalMotorHP)
    '        dMotorInertia = 0
    '        dATork = 0
    ...
    '        If Conversion.Val(CStr(ME_MRM01Car_typ.HoistCableQty)) = 0 Then
    '            dTension = 1
    '        Else
    '            dTension = T1s / (iRoping * Conversion.Val(CStr(ME_MRM01Car_typ.HoistCableQty))) 'dTension = dGrooveP * dMsDia / 2
    '        End If
    '        Try
    ...
    '        WorkingRecordSet = New ADODB.Recordset
    '        WorkingRecordSet.Open("SafetyFactors", ADOConnection)
    '        Do While Not WorkingRecordSet.EOF
    '            If WorkingRecordSet("RopeSpeed").Value = dSpeed Then
    '                bFound = True
    '                If WorkingRecordSet("PassengerSF").Value > dSafety Then
    '                    iError(1) = 1
    '                    g_bErrors = True
    '                    strError(1) = "Safety Factor " & Strings.FormatNumber(WorkingRecordSet("PassengerSF").Value, 3, TriState.True) & " is greater than the calculated safety of " & Strings.FormatNumber(dSafety, 3, TriState.True) & "."
    '                End If
    '            End If
    '            If dSpeed > 1500 Then
    ...
    '        If ME_MRM01Car_typ.MachineLocation = "Basement" Then iBasement = 3
    ...
    '                End If
    '        End Select
    '        If bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    ...
    '        If dMH <> 0 Then
    '            sSQL = "SELECT MinHP,MaxHP FROM [HP Nominal] WHERE "
    '            sSQL = sSQL & "Make = '" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '            sSQL = sSQL & "Model = '" & ME_MRM01Car_typ.MachineModel & "' AND "
    '            sSQL = sSQL & "RPM=" & CStr(Conversion.Val(ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False)))
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            If Not WorkingRecordSet.EOF Then
    '                If dMH < WorkingRecordSet("MinHP").Value Then
    '                    dMH = WorkingRecordSet("MinHP").Value
    '                    MN_TRQ01_typ.NominalMotorHP = CStr(dMH)
    '                ElseIf dMH > WorkingRecordSet("MaxHP").Value Then
    '                    iError(17) = 1
    '                    g_bErrors = True
    '                    strError(17) = "Not Standard Application. Calculated Horsepower of " & dMH & " is too large for selected machine (" & ME_MRM01Car_typ.MachineModel & ")."
    '                End If
    '            End If
    '            WorkingRecordSet.Close()
    '            WorkingRecordSet = Nothing
    '            sSQL = "SELECT * FROM [HP Limits] WHERE "
    '            sSQL = sSQL & "Manufacturer = '" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '            sSQL = sSQL & "MachineModel = '" & ME_MRM01Car_typ.MachineModel & "'"
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            If Not WorkingRecordSet.EOF Then
    '                If WorkingRecordSet("HP Limitation").Value < dMH Then
    '                    iError(16) = 1
    '                    strError(16) = "Calculated motor HP (" & Format(dMH, "0.00") & ") exceeds the machine limitation (" & Format(WorkingRecordSet("HP Limitation").Value, "0.00") & ")."
    '                    g_bErrors = True
    '                End If
    '            End If
    '            WorkingRecordSet.Close()
    ...
    '            MessageHandler(ENGINEERING_FORMS, MN_TRQ01_MENU_INDEX, IIf(strError(12) <> "", strError(12), strError(11)), MsgBoxStyle.Critical)
    '            Dependents.ParentFlag = False
    '            bTorqueCriticalError = True
    '            MN_TRQ01_typ.bPowerData = False
    '        ElseIf GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    ...
    '            IT1 = 8.8
    '        Else
    '            IT1 = (5.4 * 1000 / (ME_COM01Car_typ.PowerSupply * (3 ^ 0.5)))
    '        End If
    '        WorkingRecordSet = New ADODB.Recordset
    '        WorkingRecordSet.Open("[HP RPM MotorJ]", ADOConnection)
    '        Dim bTEandTPF As Boolean = False
    '        Do While Not WorkingRecordSet.EOF
    '            If WorkingRecordSet("RPM").Value = MN_TRQ01_typ.MotorRPM Then
    '                If WorkingRecordSet("HP").Value = dMH Then
    '                    For i As Integer = 1 To 6
    '                        aTE(i - 1) = WorkingRecordSet("te" & Conversion.Str(i).Trim()).Value
    '                        aTI(i - 1) = WorkingRecordSet("ti" & Conversion.Str(i).Trim()).Value
    '                        aTPF(i - 1) = WorkingRecordSet("tpf" & Conversion.Str(i).Trim()).Value
    '                    Next i
    '                    bTEandTPF = True
    '                    Exit Do
    '                End If
    ...
    '            If ENG_ACCESS Then
    '                MessageHandler(ENGINEERING_FORMS, MN_TRQ01_MENU_INDEX, "Based on RPM (" & MN_TRQ01_typ.MotorRPM & ") and HP (" & dMH & "), cannot calculate Motor Efficiency",
    ...
    '        MN_PowerData_GEARED_typ.ACCurrentIntoDrive_RFLU = ((Conversion.Val(MN_PowerData_GEARED_typ.CurrentIntoConverter_RFLU) * Conversion.Val(MN_TRQ01_typ.InputVoltage) / ME_COM01Car_typ.PowerSupply) + IT1).ToString
    '        MN_PowerData_GEARED_typ.LineCurrent_RFLU = MN_PowerData_GEARED_typ.ACCurrentIntoDrive_RFLU
    '        MN_PowerData_GEARED_typ.LinePower_RFLU = CStr(3 ^ 0.5 * ME_COM01Car_typ.PowerSupply * Conversion.Val(MN_PowerData_GEARED_typ.LineCurrent_RFLU) * dPOWER_FACTOR_Running / 1000.0#)
    ...
    '        dPreg = (9.80665 * dCapacity / 2.204623) * (1 - Conversion.Val(MN_TRQ01_typ.Counterweight) / 100) * ((ME_COM01Car_typ.Speed / 60) / 3.2808399) * Conversion.Val(MN_TRQ01_typ.HatchEfficiency) * dMOTOR_EFFICIENCY_Running * Conversion.Val(MN_TRQ01_typ.ForwardGearEfficiency) / 1000
    ...
    '        MN_PowerData_GEARED_typ.ACCurrentIntoDrive_AFLU = ((Conversion.Val(MN_PowerData_GEARED_typ.CurrentIntoConverter_AFLU) * Conversion.Val(MN_TRQ01_typ.InputVoltage) / ME_COM01Car_typ.PowerSupply) + IT1).ToString
    '        MN_PowerData_GEARED_typ.LineCurrent_AFLU = MN_PowerData_GEARED_typ.ACCurrentIntoDrive_AFLU
    '        MN_PowerData_GEARED_typ.LinePower_AFLU = CStr(3 ^ 0.5 * ME_COM01Car_typ.PowerSupply * Conversion.Val(MN_PowerData_GEARED_typ.LineCurrent_AFLU) * dPOWER_FACTOR_Accelerating / 1000.0#)
    ...
    '        sDriveSelection = DriveSelection(dIrflu, dIaflu, dPreg, Strings.Right(ME_MRM01Car_typ.ControllerType, 3) = "PF1")
    '        sDriveSelection = DriveSelectionValidation(dIrflu, dIaflu, dPreg, Strings.Right(ME_MRM01Car_typ.ControllerType, 3) = "PF1")
    ...
    '            MN_PowerData_GEARED_typ.ACCurrentIntoDrive_RFLU = ((Conversion.Val(MN_PowerData_GEARED_typ.CurrentIntoConverter_RFLU) * Conversion.Val(MN_TRQ01_typ.InputVoltage) / ME_COM01Car_typ.PowerSupply) + IT1).ToString
    '            MN_PowerData_GEARED_typ.LineCurrent_RFLU = MN_PowerData_GEARED_typ.ACCurrentIntoDrive_RFLU
    '            MN_PowerData_GEARED_typ.LinePower_RFLU = CStr(3 ^ 0.5 * ME_COM01Car_typ.PowerSupply * Conversion.Val(MN_PowerData_GEARED_typ.LineCurrent_RFLU) * dPOWER_FACTOR_Running / 1000)
    '            'MOTOR_EFFICIENCY =2nd eq
    '            dMOTOR_EFFICIENCY_Accelerating = aTE(0)
    '            dIaflu = aTI(0)
    '            For i As Integer = 2 To 6
    ...
    '            MN_PowerData_GEARED_typ.ACCurrentIntoBuilding_TSFLD = CStr((Conversion.Val(MN_PowerData_GEARED_typ.CurrentOutOfTheConverter_TSFLD)) * Conversion.Val(MN_TRQ01_typ.InputVoltage) / ME_COM01Car_typ.PowerSupply)
    '            MN_PowerData_GEARED_typ.LineCurrent_TSFLD = MN_PowerData_GEARED_typ.ACCurrentIntoBuilding_TSFLD
    '            MN_PowerData_GEARED_typ.LinePower_TSFLD = CStr(3 ^ 0.5 * ME_COM01Car_typ.PowerSupply * Conversion.Val(MN_PowerData_GEARED_typ.LineCurrent_TSFLD) * PF4 / 1000)
    ...
    '            MN_PowerData_GEARED_typ.ACCurrentIntoBuilding_TRFLU = CStr((Conversion.Val(MN_PowerData_GEARED_typ.CurrentOutOfTheConverter_TRFLU)) * Conversion.Val(MN_TRQ01_typ.InputVoltage) / ME_COM01Car_typ.PowerSupply)
    '            MN_PowerData_GEARED_typ.LineCurrent_TRFLU = MN_PowerData_GEARED_typ.ACCurrentIntoBuilding_TRFLU
    '            MN_PowerData_GEARED_typ.LinePower_TRFLU = CStr(3 ^ 0.5 * ME_COM01Car_typ.PowerSupply * Conversion.Val(MN_PowerData_GEARED_typ.LineCurrent_TRFLU) * PF3 / 1000)
    ...
    '        Dim EP As Single = ME_COM01Car_typ.PowerSupply
    '        Dim iVFLU As Integer = Math.Round(Conversion.Val(VFLU))

    '        Select Case ME_MRM01Car_typ.ControllerType
    '            Case DRIVETYPE_BESTPRICE, DRIVETYPE_BR, DRIVETYPE_PF1
    '                EP = Math.Round(MN_TRQ01_typ.UN / ME_COM01Car_typ.PowerSupply, 2)
    '        End Select
    '        IDC1 = ME_MRM01Car_typ.NamePlateArmatureFullLoadCurrent.ToString
    ...
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open("E1", ADOConnection)
    '            Do While Not WorkingRecordSet.EOF
    '                ReDim Preserve aE1(aE1.GetUpperBound(0) + 1)
    '                aE1(aE1.GetUpperBound(0)) = WorkingRecordSet("Voltage").Value
    '                WorkingRecordSet.MoveNext()
    '            Loop
    '            WorkingRecordSet.Close()
    '            WorkingRecordSet = Nothing
    ...
    '                PF4 = 1.0#
    '        End Select
    '        If ME_COM01Car_typ.CabApplication = "General Purpose" And ME_COM01Car_typ.Performance = StandardPerformance Then
    '            DutyFactor = 0.83
    '        Else
    '            DutyFactor = 1.0#
    '        End If
    '        E2 = 170
    '        If Conversion.Val(ME_MRM01Car_typ.VFF) >= (0.8 * E2) Then
    '            E2 = 340
    '        End If
    '        Select Case ME_MRM01Car_typ.ControllerType
    '            Case DRIVETYPE_SCRDC
    '                IAC1 = (DutyFactor * E1 * Conversion.Val(IDC1) * 0.85) / EP + E2 * Conversion.Val(ME_MRM01Car_typ.IFF) * 0.75 / EP + Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan) / (Math.Sqrt(3) * EP) + IT1
    '                IAC2 = (DutyFactor * E1 * Conversion.Val(IDC2) * 0.85) / EP + E2 * Conversion.Val(ME_MRM01Car_typ.IFF) * 0.75 / EP + Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan) / (Math.Sqrt(3) * EP) + IT1
    ...
    '            Case DRIVETYPE_QUATTRO
    '                IAC1 = (Conversion.Val(VFLU) * Conversion.Val(IDC1) * 1.1) / (EP * 3 ^ 0.5) + IT1 + (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '                IAC2 = (Conversion.Val(VFLU) * Conversion.Val(IDC2) * 1.1) / (EP * 3 ^ 0.5) + IT1 + (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '                IAC3 = (Conversion.Val(VFLU) * Conversion.Val(IDC3) * 1.1) / (EP * 3 ^ 0.5) - IT1 - (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '                IAC4 = (Conversion.Val(VFLU) * Conversion.Val(IDC4) * 1.1) / (EP * 3 ^ 0.5) - IT1 - (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '                IAC5 = (Conversion.Val(VFLU) * Conversion.Val(IDC5) * 1.1) / (EP * 3 ^ 0.5) - IT1 - (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '                IAC6 = (Conversion.Val(VFLU) * Conversion.Val(IDC6) * 1.1) / (EP * 3 ^ 0.5) - IT1 - (Conversion.Val(ME_MRM01Car_typ.VFan) * Conversion.Val(ME_MRM01Car_typ.IFan)) / (EP * Math.Sqrt(3))
    '        End Select
    ...
    '        End If
    '        sDriveSelection = DriveSelectionGEARLESS(ME_MRM01Car_typ.ControllerType = DRIVETYPE_SCRDC)
    ...
    '                End If
    '            End If
    '        ElseIf ME_MRM01Car_typ.ControllerType = DRIVETYPE_SCRDC Then
    '            Dim MinKVA As Single = DutyFactor * 0.85 * Conversion.Val(IDC1) * E1 * 3 ^ 0.5 / 1000
    '            Dim K1 As Single = Math.Ceiling(MinKVA)
    '            Dim K2 As Single = E2 * Conversion.Val(ME_MRM01Car_typ.IFF) / 1000
    '            KT = K1 + K2
    '        End If
    ...
    '            dMsDia = Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia)
    '            dRopeOffset = Conversion.Val(MN_TRQ01_typ.RopeOffset)
    '            If dX1 = 0 Or dY1 = 0 Or dDsDia = 0 Or dMsDia = 0 Or (dRopeOffset = 0 And ME_MRM01Car_typ.MachineLocation <> "Overhead") Then
    ...
    '                    bCriteria = GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE
    '                    If bCriteria Then bCriteria = ME_MRM01Car_typ.Wrap = "Double Wrap"
    '                    If bCriteria Then
    '                        dWA = 270 + b + 3 * C
    '                    Else
    '                        dWA = 90 + b + C
    ...
    '        Select Case GONumbers(CurrentGOSelection).MachineType
    '            Case GEARED_TYPE
    '                Select Case ME_COM01Car_typ.Performance
    ...
    '        If ME_MRM01Car_typ.Wrap = "Single Wrap" Then
    ...
    '        Dim sSQL As String = ""
    '        Dim bParentFlag As Boolean = Dependents.ParentFlag

    '        result = ""
    '        If MN_TRQ01_typ.DriveModel = "" Or MN_TRQ01_typ.DriveModelDefault = "" Or bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    '            sSQL = "[Drive Selection " & (IIf(bSSDC, "SSDC", "Quattro")) & "]"
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            Do While Not WorkingRecordSet.EOF
    '                If WorkingRecordSet("IDC1").Value >= Conversion.Val(IDC1) Then
    '                    If WorkingRecordSet("IDC2").Value >= Conversion.Val(IDC2) Then
    '                        If Conversion.Val(ME_MRM01Car_typ.IFF) < WorkingRecordSet("IFFmin").Value Or Conversion.Val(ME_MRM01Car_typ.IFF) > WorkingRecordSet("IFFmax").Value Then
    '                            MN_TRQ01_typ.DriveModel = "IFF Error"
    '                        End If
    '                        MN_TRQ01_typ.DriveModelDefault = WorkingRecordSet("Drive Selection").Value
    '                        If MN_TRQ01_typ.DriveModel = "" Or bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    ...
    '        result = ""
    '        If MN_TRQ01_typ.DriveModel = "" Or MN_TRQ01_typ.DriveModelDefault = "" Or bMotorDependentChange Or
    '           Dependents.StatusChangeCarWeightAvailable Then
    '            sSQL = "SELECT * From [Drive Selection] WHERE ANSI=" & (IIf(bANSI, "True", "False")) & " ORDER BY Sequence"
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            Do While Not WorkingRecordSet.EOF
    '                If WorkingRecordSet("Irflu").Value >= dIrflu And WorkingRecordSet("Drive Selection").Value.ToString <> "VF85BR" And
    '                   ((ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR And Strings.Right(WorkingRecordSet("Drive Selection").Value.ToString, 2) = "BR") Or
    '                   (ME_MRM01Car_typ.ControllerType = DRIVETYPE_PF1 And Strings.Right(WorkingRecordSet("Drive Selection").Value.ToString, 3) = "PF1")) Then
    '                    If WorkingRecordSet("Iaflu").Value >= dIaflu Then
    '                        bCriteria = False
    '                        If Right(WorkingRecordSet("Drive Selection").Value, 3) = "PF1" Then
    '                            If WorkingRecordSet("IDFLD").Value >= Math.Abs(Conversion.Val(MN_PowerData_GEARED_typ.CurrentOutOfTheConverter_TSFLD)) Then
    '                                bCriteria = True
    '                            End If
    '                        Else
    '                            If WorkingRecordSet("Preg").Value >= dPreg / 1000 Then
    '                                bCriteria = True
    '                            End If
    '                        End If
    '                        If bCriteria Then
    '                            MN_TRQ01_typ.Required_KVA = CalculateTransformerKVA(WorkingRecordSet("Required_KVA").Value) 'transformer KVA
    '                            dRequired_AMPS = 1000 * (MN_TRQ01_typ.Required_KVA + K3) / (3 ^ 0.5 * ME_COM01Car_typ.PowerSupply)
    '                            If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '                                MN_PowerData_GEARED_typ.PeakCurrent = CStr(1000 * MN_TRQ01_typ.Required_KVA / (3 ^ 0.5 * ME_COM01Car_typ.PowerSupply) * 10)
    '                            End If
    '                            MN_TRQ01_typ.DriveModelDefault = WorkingRecordSet("Drive Selection").Value
    '                            If MN_TRQ01_typ.DriveModel = "" Or bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    '                                MN_TRQ01_typ.DriveModel = MN_TRQ01_typ.DriveModelDefault
    ...
    '                MN_PowerData_GEARED_typ.PeakCurrent = CStr(0)
    '            Case Else
    '                bANSI = (ME_ADM01Bnk_typ.ANSICode) >= 2000
    '                If Dependents.StatusChangeDrive Or MN_TRQ01_typ.DriveModel <> "" Then
    ...
    '                WorkingRecordSet.Open(sSQL, ADOConnection)
    '                Do While Not WorkingRecordSet.EOF
    '                    If WorkingRecordSet("Irflu").Value >= dIrflu Then
    '                        If WorkingRecordSet("Iaflu").Value >= dIaflu Then
    '                            bCriteria = False
    '                            If bPF1 Then
    '                                If WorkingRecordSet("IDFLD").Value >= Math.Abs(Conversion.Val(MN_PowerData_GEARED_typ.CurrentOutOfTheConverter_TSFLD)) Then bCriteria = True
    '                            Else
    '                                If WorkingRecordSet("Preg").Value >= dPreg / 1000 Then bCriteria = True
    '                            End If
    '                            If bCriteria Then
    '                                MN_TRQ01_typ.Required_KVA = CalculateTransformerKVA(WorkingRecordSet("Required_KVA").Value) 'transformer KVA
    '                                dRequired_AMPS = 1000 * (MN_TRQ01_typ.Required_KVA + K3) / (3 ^ 0.5 * ME_COM01Car_typ.PowerSupply)
    ...
    '        Dim dOffset, k, U, dWAgiven, dWAcalculated As Double

    '        Try

    '            If Conversion.Val(MN_TRQ01_typ.RopeDrop) = 0 Then Return result
    '            If Conversion.Val(MN_TRQ01_typ.WrapAngle) <= 0 Then Return result
    '            dDsDia = HoistwaySheaveDiameter()
    '            dMsDia = Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia)
    ...
    '        Dim sSQL As String = "SELECT * FROM [HP Limits] WHERE "
    '        sSQL = sSQL & "Manufacturer = '" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '        sSQL = sSQL & "MachineModel = '" & ME_MRM01Car_typ.MachineModel & "'"
    '        WorkingRecordSet = New ADODB.Recordset
    '        WorkingRecordSet.Open(sSQL, ADOConnection)
    '        If Not WorkingRecordSet.EOF Then
    '            If WorkingRecordSet("HP Limitation").Value < dHPower Then
    '                iError(16) = 1
    '                strError(16) = "Calculated motor HP (" & Format(dHPower, "0.00") & ") exceeds the machine limitation (" & Format(WorkingRecordSet("HP Limitation").Value, "0.00") & ")."
    '                g_bErrors = True
    '            End If
    '        End If
    '        WorkingRecordSet.Close()
    '        WorkingRecordSet = Nothing
    '        Dim T As String = ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, True)
    '        If T <> "" Then
    '            sSQL = "SELECT HP FROM [HP Criterias] WHERE "
    '            sSQL = sSQL & "Make = '" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '            sSQL = sSQL & "Model = '" & ME_MRM01Car_typ.MachineModel & "' AND "
    '            sSQL = sSQL & "Machine = 'New' AND "
    '            sSQL = sSQL & "RPM " & T
    '            sSQL = sSQL & " ORDER BY HP"
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            If Not WorkingRecordSet.EOF Then
    '                Do While Not WorkingRecordSet.EOF
    '                    If WorkingRecordSet("HP").Value >= dHPower Then
    '                        dHPowerRounded = WorkingRecordSet("HP").Value
    '                        Exit Do
    '                    End If
    '                    WorkingRecordSet.MoveNext()
    '                Loop
    '                If WorkingRecordSet.EOF Then
    '                    iError(15) = 1
    ...
    '            T = ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False)
    '            sSQL = "SELECT MinHP,MaxHP FROM [HP Nominal] WHERE "
    '            sSQL = sSQL & "Make = '" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '            sSQL = sSQL & "Model = '" & ME_MRM01Car_typ.MachineModel & "' AND "
    '            sSQL = sSQL & "RPM=" & T
    ...
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            If Not WorkingRecordSet.EOF Then
    '                If Not IsNothing(WorkingRecordSet("MinHP").Value) And Not IsNothing(WorkingRecordSet("MaxHP").Value) Then
    '                    If Math.Round(WorkingRecordSet("MinHP").Value) > Math.Round(Conversion.Val(MN_TRQ01_typ.NominalMotorHP)) Or
    '                       Math.Round(WorkingRecordSet("MaxHP").Value) < Math.Round(Conversion.Val(MN_TRQ01_typ.NominalMotorHP)) Then
    '                        iError(15) = 2
    ...
    '                    ElseIf ME_MRM01Car_typ.MachineModelNew = MACHINE_PMR490 And ME_MRM01Car_typ.ControllerType = DRIVETYPE_PF1 Then
    '                        MN_TRQ01_typ.DriveModelDefault = "VF155PF1"
    '                    ElseIf ME_MRM01Car_typ.MachineModelNew = MACHINE_FMR355 And ME_MRM01Car_typ.ControllerType = DRIVETYPE_PF1 Then
    '                        MN_TRQ01_typ.DriveModelDefault = "VF288PF1"
    '                    ElseIf ME_MRM01Car_typ.MachineModelNew = MACHINE_PMR355 And ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR Then
    '                        MN_TRQ01_typ.DriveModelDefault = "VF122BR"
    '                    ElseIf ME_MRM01Car_typ.MachineModelNew = MACHINE_FM630 And ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR Then
    '                        MN_TRQ01_typ.DriveModelDefault = "VF122BR"
    '                    Else
    ...
    '                        If FormsArray(ENGINEERING_FORMS, MN_TRQ02_MENU_INDEX).FormIsCompleted And ENG_ACCESS Then
    '                            MessageHandler(ENGINEERING_FORMS, MN_TRQ01_MENU_INDEX, "Changed Drive Model from " & MN_TRQ01_typ.DriveModel & " to " & _
    '                                       MN_TRQ01_typ.DriveModelDefault, MsgBoxStyle.Critical)
    '                        End If
    '                        MN_TRQ01_typ.DriveModel = MN_TRQ01_typ.DriveModelDefault
    '                    End If
    '                End If
    ...
    '                Select_RopeGripperModel()
    '            Else
    '                If Not FormsArray(ENGINEERING_FORMS, MN_TRQ02_MENU_INDEX).FormIsCompleted Or MN_TRQ01_typ.bDirty Then
    '                    PreTorque()
    '                End If
    '                CalculateTORQUE()
    '                Torque2DebugFile()
    ...
    '        Dim VFF, VFan, VFLU, IDC1, IFF, IFan, tempIdlerSheaveDiameter, tempNominalMotorRPM As String
    ...
    '                VFF = CStr(ME_MRM01Car_typ.VFF)
    '                VFLU = CStr(ME_MRM01Car_typ.NamePlateArmatureFullLoadVoltage)
    '                IDC1 = CStr(ME_MRM01Car_typ.NamePlateArmatureFullLoadCurrent)
    '                IFF = ME_MRM01Car_typ.IFF
    '                VFan = ME_MRM01Car_typ.VFan
    '                IFan = ME_MRM01Car_typ.IFan
    '                tempNominalMotorRPM = MN_TRQ01_typ.NomimalMotorRpm
    '                MN_TRQ01_typ = MN_TRQ01_typDec.CreateInstance()
    '                MN_TRQ01_typ.NomimalMotorRpm = tempNominalMotorRPM
    '                Dependents.Set_SoftwareLevel()
    '                MN_TRQ01_typ.bDirty = True
    '                ME_MRM01Car_typ.VFF = VFF
    '                ME_MRM01Car_typ.VFan = VFan
    '                ME_MRM01Car_typ.NamePlateArmatureFullLoadVoltage = CSng(VFLU)
    '                ME_MRM01Car_typ.NamePlateArmatureFullLoadCurrent = CSng(IDC1)
    '                ME_MRM01Car_typ.IFF = IFF
    '                ME_MRM01Car_typ.IFan = IFan
    ...
    '                End If
    '            End If
    '            Dependents.Set_CarWeight()
    '            Select Case ME_CAR01Car_typ.TopOfCarSheave
    '                Case "Replace", "New"
    '                    If Conversion.Val(MN_TRQ01_typ.CarSheaveDiameter) = 0 Or MN_TRQ01_typ.bDirty Then
    '                        If ME_CAR01Car_typ.TopOfCarSheave <> "None" Then
    '                            MN_TRQ01_typ.CarSheaveDiameter = ME_CAR01Car_typ.CarSheaveDiameter
    '                        Else
    '                            MN_TRQ01_typ.CarSheaveDiameter = CStr(0)
    '                        End If
    '                    End If
    '                    If Conversion.Val(MN_TRQ01_typ.CarSheaveDiameter) <> 0 And Conversion.Val(MN_TRQ01_typ.CarSheaveJ) = 0 Then Dependents.StatusChangeCarTopSheaveDiameterCalculated = True
    '                    Dependents.Set_CarSheaveJ()
    ...
    '                            MN_TRQ01_typ.NumberofCarTopSheaves = CStr(2)
    '                        Else
    '                            MN_TRQ01_typ.NumberofCarTopSheaves = CStr(ME_CAR01Car_typ.CarSheaveQty)
    '                        End If
    '                    End If
    '                Case "Reuse", "Refurbish", "Relocate"
    '                    If Dependents.StatusChangeTopOfCar Or Dependents.StatusChangeCarSheaveDiameter Or Conversion.Val(MN_TRQ01_typ.CarSheaveDiameter) = 0 Then
    '                        MN_TRQ01_typ.CarSheaveDiameter = ME_CAR01Car_typ.CarSheaveDiameter
    '                    End If
    '                    If Conversion.Val(MN_TRQ01_typ.CarSheaveDiameter) <> 0 And Conversion.Val(MN_TRQ01_typ.CarSheaveJ) = 0 Then Dependents.StatusChangeCarTopSheaveDiameterCalculated = True
    '                    Dependents.Set_CarSheaveJ()
    ...
    '                    MN_TRQ01_typ.NumberofCarTopSheaves = CStr(ME_CAR01Car_typ.CarSheaveQty)
    '                Case "None"
    '                    MN_TRQ01_typ.CarSheaveDiameter = ""
    '                    MN_TRQ01_typ.CarSheaveJ = ""
    '                    MN_TRQ01_typ.NumberofCarTopSheaves = ""
    '            End Select
    '            Select Case ME_HstWay02Car_typ.HWCwtSheave
    '                Case "Replace", "New"
    '                    If Conversion.Val(MN_TRQ01_typ.CwtSheaveDiameter) = 0 Or MN_TRQ01_typ.bDirty Then
    '                        If ME_HstWay02Car_typ.HWCwtSheave <> "None" Then
    '                            MN_TRQ01_typ.CwtSheaveDiameter = ME_HstWay02Car_typ.HWCwtSheaveDia
    '                        Else
    '                            MN_TRQ01_typ.CwtSheaveDiameter = CStr(0)
    '                        End If
    '                    End If
    '                    If Conversion.Val(MN_TRQ01_typ.CwtSheaveDiameter) <> 0 And Conversion.Val(MN_TRQ01_typ.CwtSheaveJ) = 0 Then Dependents.StatusChangeCwtTopSheaveDiameterCalculated = True
    '                    Dependents.Set_CwtSheaveJ()
    '                    Dependents.StatusChangeCwtTopSheaveDiameterCalculated = False
    '                Case "Reuse", "Refurbish", "Relocate"
    '                    If Dependents.StatusChangeHoistwaySheave Or Dependents.StatusChangeHWCwtSheaveDia Or Conversion.Val(MN_TRQ01_typ.CwtSheaveDiameter) = 0 Then
    '                        MN_TRQ01_typ.CwtSheaveDiameter = ME_HstWay02Car_typ.HWCwtSheaveDia
    '                    End If
    '                    If (MN_TRQ01_typ.CwtSheaveDiameter) <> 0 And Conversion.Val(MN_TRQ01_typ.CwtSheaveJ) = 0 Then Dependents.StatusChangeCwtTopSheaveDiameterCalculated = True
    '                    Dependents.Set_CwtSheaveJ()
    '                    Dependents.StatusChangeCwtTopSheaveDiameterCalculated = False
    '                Case "None"
    '                    MN_TRQ01_typ.CwtSheaveDiameter = ""
    '                    MN_TRQ01_typ.CwtSheaveJ = ""
    '            End Select
    '            If Dependents.OnlyBasement = SET_TO_ACTIVE Then
    '                If MN_TRQ01_typ.bDirty Or (Conversion.Val(MN_TRQ01_typ.IdlerSheaveDiameter) = 0 And Dependents.StatusChangeIdlerSheaveDiameterCalculated) Then
    '                    tempIdlerSheaveDiameter = CStr(Conversion.Val(TranslateRopeSize(ME_MRM01Car_typ.HoistCableSize, True)) * 40)
    ...
    '                    End If
    '                End If
    '                If MN_TRQ01_typ.bDirty Or Dependents.StatusChangeIdlerSheaveDiameterCalculated Or Conversion.Val(MN_TRQ01_typ.OverheadSheaveInertia) = 0 Then
    '                    Dependents.StatusChangeIdlerSheaveDiameterCalculated = True
    '                    Dependents.Set_OverheadSheaveInertia()
    '                    Dependents.StatusChangeIdlerSheaveDiameterCalculated = False
    '                End If
    '                If Conversion.Val(MN_TRQ01_typ.NumberofIdlerSheaves) = 0 Then
    '                    MN_TRQ01_typ.NumberofIdlerSheaves = CStr(4)
    '                End If
    '            Else
    ...
    '            End If
    '            'Groove Number
    '            Dependents.Set_GrooveNumber()
    '            'Machine
    '            UpdateGearData()
    '            Dependents.Set_GearRatio2()
    '            'Travel Cable
    '            If MN_TRQ01_typ.MainTravelingCable = "" Then Dependents.Set_MainTravelingCable()
    '            If MN_TRQ01_typ.MainTravelingCableQty = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.MainTravelingCableQty = "1"
    '            Dependents.Set_ControlCableWeight()
    '            'RopeStrength
    '            Dependents.Set_RopeStrength()
    '            'Compensation  Data
    '            If MN_TRQ01_typ.SheaveDiameter = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.SheaveDiameter = ME_PIT01Car_typ.SheaveDiameter
    '            Dependents.Set_CompSheaveJandWt()
    '            If Conversion.Val(MN_TRQ01_typ.CompensationWeight) = 0 Or MN_TRQ01_typ.bDirty Then
    '                If (ME_PIT01Car_typ.Compensation = "Reuse" Or ME_PIT01Car_typ.Compensation = "Relocate") And ME_PIT01Car_typ.CableChainSize = "Unknown" Then
    '                    Dependents.Set_RopeStrength()
    '                    dRWT = Conversion.Val(CStr(ME_MRM01Car_typ.HoistCableQty)) * Conversion.Val(MN_TRQ01_typ.RopeWeight)
    ...
    '                    MN_TRQ01_typ.CompensationWeight = Strings.FormatNumber(dIdealCompWeight / 2, 3, TriState.True)
    '                Else
    '                    MN_TRQ01_typ.CompensationWeight = CStr(ME_PIT01Car_typ.CableChainQty * Get_CompUnitWeight())
    '                End If
    '            End If
    '            'Wrap Angle Calculations
    '            If Conversion.Val(MN_TRQ01_typ.WrapAngle) = 0 Or MN_TRQ01_typ.bDirty Then Set_WrapAngle()
    '            'Other
    '            Calc_GeneralData()
    '            If MN_TRQ01_typ.HatchEfficiency = "" Or MN_TRQ01_typ.bDirty Then InitHatchEff()
    '            'Rope Modifiers
    ...
    '                    End If
    '                End If
    '                If MN_TRQ01_typ.len2 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.len2 = "30"
    '                If MN_TRQ01_typ.len3 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.len3 = "3"
    '            Else
    '                If MN_TRQ01_typ.len1 = "" Or MN_TRQ01_typ.bDirty Then
    '                    If ME_MRM01Car_typ.RopingNew = "1:1" Then
    ...
    '                End If
    '                If MN_TRQ01_typ.len2 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.len2 = "30"
    '                If MN_TRQ01_typ.len3 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.len3 = "12"
    '            End If
    '            If MN_TRQ01_typ.RopeGrade = "" Or MN_TRQ01_typ.bDirty Then
    '                If TranslateRopeSize(ME_MRM01Car_typ.HoistCableSize, True) < 0.5 Then
    ...
    '                If Dependents.DeflectorSheaveDiameter = SET_TO_ACTIVE Then
    '                    If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '                        MN_TRQ01_typ.DeflectorSheaveDiameter = ME_HstWay02Car_typ.HoistwaySheaveDia
    '                    Else
    '                        Select Case ME_HstWay02Car_typ.HoistwaySheave
    '                            Case "Reuse", "Regroove", "Relocate"
    '                                MN_TRQ01_typ.DeflectorSheaveDiameter = ME_MRM01Car_typ.MachineSheaveDia
    '                            Case "New", "Replace"
    '                                MN_TRQ01_typ.DeflectorSheaveDiameter = ME_HstWay02Car_typ.HoistwaySheaveDia
    '                        End Select
    '                    End If
    '                    Dependents.Set_DeflectorSheaveJ()
    '                Else
    ...
    '        MN_TRQ01_typ.MotorRPM = Conversion.Val(Conversion.Val(ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False)))
    '        Select Case ME_MRM01Car_typ.MachineModel
    '            Case "18" : HSSInertiaAdder = 0.0468
    '            Case "28" : HSSInertiaAdder = 0.0426
    '            Case "37", "57" : HSSInertiaAdder = 0.0407
    '            Case "38" : HSSInertiaAdder = 0.0432
    '            Case "58" : HSSInertiaAdder = 0.032
    '            Case "61" : HSSInertiaAdder = 0.0471
    '            Case "HW43/44" : HSSInertiaAdder = IIf(ME_MRM01Car_typ.BrakeType = "Drum", -0.0019, 0.0184)
    '            Case "HW53/54" : HSSInertiaAdder = IIf(ME_MRM01Car_typ.BrakeType = "Drum", 0.0375, 0.0184)
    '            Case "HW63/64" : HSSInertiaAdder = IIf(ME_MRM01Car_typ.BrakeType = "Drum", 0.0375, 0.0229)
    '            Case "HW74" : HSSInertiaAdder = IIf(ME_MRM01Car_typ.BrakeType = "Drum", 0.0003, 0.0229)
    '            Case Else : HSSInertiaAdder = 0
    '        End Select
    ...
    '            End If
    '            If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '                If ME_MRM01Car_typ.BrakeType = "Disk" Or bPretend Then
    '                    sSQL = "SELECT DISTINCT [HSS Inertia] From SuperTable "
    '                    If ME_MRM01Car_typ.MachineModel = OTHER Or ME_MRM01Car_typ.MachineModel = "" Then
    '                        If ME_MRM01Car_typ.MachineVendor = OTHER Or ME_MRM01Car_typ.MachineVendor = "" Then
    '                            Select Case ME_MRM01Car_typ.RopingNew
    '                                Case "1:1"
    '                                    If ME_COM01Car_typ.Capacity <= 2000 Then
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.087"
    '                                    ElseIf ME_COM01Car_typ.Capacity > 4000 Then
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.214"
    '                                    Else
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.112"
    '                                    End If
    '                                Case Else
    '                                    If ME_COM01Car_typ.Capacity <= 4000 Then
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.087"
    '                                    ElseIf ME_COM01Car_typ.Capacity > 8000 Then
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.214"
    '                                    Else
    '                                        aMachineData(0).HiSpeedShaftInertia = "0.112"
    '                                    End If
    '                            End Select
    '                        Else
    '                            sWhere = "WHERE Manufacturer='" & ME_MRM01Car_typ.MachineVendor & "' "
    '                        End If
    '                    Else
    '                        sWhere = "WHERE [Brake Drum/Disk]=" & (IIf(ME_MRM01Car_typ.BrakeType = "Disk", 2, 1)) & " AND Manufacturer='" & ME_MRM01Car_typ.MachineVendor & "' AND MachineModel='" & ME_MRM01Car_typ.MachineModel & "'"
    '                    End If
    '                    sSQL = sSQL & sWhere & " ORDER BY [HSS Inertia] DESC;"
    '                    WorkingRecordSet = New ADODB.Recordset
    '                    WorkingRecordSet.Open(sSQL, ADOConnection)
    '                    If Not WorkingRecordSet.EOF Then
    '                        Select Case WorkingRecordSet("HSS Inertia").Value
    '                            Case 1 : aMachineData(0).HiSpeedShaftInertia = "0.2571"
    '                            Case 2 : aMachineData(0).HiSpeedShaftInertia = "0.6405"
    '                            Case 3 : aMachineData(0).HiSpeedShaftInertia = "1.3251"
    '                            Case 4 : aMachineData(0).HiSpeedShaftInertia = "3.234"
    '                            Case Else : aMachineData(0).HiSpeedShaftInertia = WorkingRecordSet("HSS Inertia").Value
    '                        End Select
    ...
    '                    Select Case ME_MRM01Car_typ.RopingNew
    '                        Case "1:1"
    '                            If ME_COM01Car_typ.Capacity <= 4500 Then
    '                                aMachineData(0).HiSpeedShaftInertia = "0.6405"
    '                            ElseIf ME_COM01Car_typ.Capacity > 6000 Then
    '                                aMachineData(0).HiSpeedShaftInertia = "3.2340"
    '                            Else
    '                                aMachineData(0).HiSpeedShaftInertia = "1.3251"
    '                            End If
    '                        Case "2:1"
    '                            If ME_COM01Car_typ.Capacity <= 9000 Then
    '                                aMachineData(0).HiSpeedShaftInertia = "0.6405"
    '                            ElseIf ME_COM01Car_typ.Capacity > 12000 Then
    '                                aMachineData(0).HiSpeedShaftInertia = "3.2340"
    '                            Else
    '                                aMachineData(0).HiSpeedShaftInertia = "1.3251"
    '                            End If
    ...
    '                bCriteria = True
    '                If ME_MRM01Car_typ.MachineModel <> OTHER Then
    '                    sSQL = "SELECT [Std Susp Ld] From SuperTable GROUP BY MachineModel, Manufacturer, [Std Susp Ld] " & "HAVING MachineModel='" & ME_MRM01Car_typ.MachineModel & "' AND Manufacturer='" & ME_MRM01Car_typ.MachineVendor & "'"
    '                    WorkingRecordSet = New ADODB.Recordset
    '                    WorkingRecordSet.Open(sSQL, ADOConnection)
    '                    If Not WorkingRecordSet.EOF Then aMachineData(0).MaxShaftSheaveLoad = WorkingRecordSet("Std Susp Ld").Value.ToString
    '                    WorkingRecordSet.Close()
    '                    WorkingRecordSet = Nothing
    '                    sSQL = "SELECT [Machine Inertia] From SuperTable GROUP BY MachineModel, Manufacturer, [Machine Inertia] " & "HAVING MachineModel='" & ME_MRM01Car_typ.MachineModel & "' AND Manufacturer='" & ME_MRM01Car_typ.MachineVendor & "'"
    '                    WorkingRecordSet = New ADODB.Recordset
    '                    WorkingRecordSet.Open(sSQL, ADOConnection)
    '                    If Not WorkingRecordSet.EOF Then
    '                        If WorkingRecordSet.RecordCount = 1 Then
    '                            If IsDBNull(WorkingRecordSet("Machine Inertia").Value) Then
    '                                aMachineData(0).MachineSheaveJ = String.Empty
    '                            Else
    '                                aMachineData(0).MachineSheaveJ = WorkingRecordSet("Machine Inertia").Value
    '                            End If
    '                            bCriteria = False
    '                        End If
    '                    End If
    ...
    '                End If
    '                If bCriteria Then
    '                    If Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia) <= 30 Then
    '                        aMachineData(0).MachineSheaveJ = CStr(17)
    '                    Else
    '                        aMachineData(0).MachineSheaveJ = CStr(IIf(Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia) > 36, 68, 34))
    '                    End If
    '                End If
    '                aMachineData(0).GearRatio1 = Format(MN_TRQ01_typ.MotorRPM * 2 * PI * (Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia) / 24) / (ME_COM01Car_typ.Speed * Conversion.Val(Strings.Left(ME_MRM01Car_typ.RopingNew, 1))), "0.####")
    '            Else
    '                sSQL = "SELECT SuperTable.[Machine Inertia], SuperTable.[Std Susp Ld], SuperTable.HP "
    '                sSQL = sSQL & "FROM SuperTable "
    '                sSQL = sSQL & "WHERE SuperTable.MachineType='" & GEARLESS_TYPE & "' "
    '                sSQL = sSQL & "AND SuperTable.Roping=" & Strings.Left(ME_MRM01Car_typ.RopingNew, 1)
    '                sSQL = sSQL & " AND CStr('' & [SuperTable].[Speed])='" & CStr(ME_COM01Car_typ.Speed) & "'"
    '                sSQL = sSQL & " AND SuperTable.Wrap=" & CStr(IIf(ME_MRM01Car_typ.Wrap = "Single Wrap", 1, 2))
    '                sSQL = sSQL & " AND SuperTable.Capacity=" & CStr(ME_COM01Car_typ.Capacity)
    '                WorkingRecordSet = New ADODB.Recordset
    '                WorkingRecordSet.Open(sSQL, ADOConnection)
    '                If Not WorkingRecordSet.EOF Then
    '                    If WorkingRecordSet("Machine Inertia").ToString = "System.__ComObject" Then
    '                        aMachineData(0).MachineSheaveJ = ""
    '                    Else
    '                        aMachineData(0).MachineSheaveJ = Format(WorkingRecordSet("Machine Inertia").Value, "0.####")
    '                    End If
    '                    If WorkingRecordSet("Std Susp Ld").ToString = "System.__ComObject" Then
    '                        aMachineData(0).MaxShaftSheaveLoad = ""
    '                    Else
    '                        aMachineData(0).MaxShaftSheaveLoad = "" & WorkingRecordSet("Std Susp Ld").Value
    '                    End If
    '                Else
    '                    sSQL = "SELECT [Std Susp Ld], [Machine Inertia] From SuperTable "
    '                    sSQL = sSQL & "GROUP BY Manufacturer, MachineModel, MachineType, [Std Susp Ld], [Machine Inertia] "
    '                    sSQL = sSQL & "HAVING Manufacturer='" & ME_MRM01Car_typ.MachineVendor & "' AND "
    '                    sSQL = sSQL & "MachineModel = '" & ME_MRM01Car_typ.MachineModel & "' AND "
    '                    sSQL = sSQL & "MachineType='Gearless'"
    '                    WorkingRecordSet.Close()
    '                    WorkingRecordSet = New ADODB.Recordset
    '                    WorkingRecordSet.Open(sSQL, ADOConnection)
    '                    If Not WorkingRecordSet.EOF Then
    ...
    '            End If
    '            Dependents.Set_DeflectorSheaveJ()
    '            If MN_TRQ01_typ.MachineSheaveJ = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.MachineSheaveJ = aMachineData(0).MachineSheaveJ
    '            If MN_TRQ01_typ.GearRation1 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.GearRation1 = aMachineData(0).GearRatio1
    '            If MN_TRQ01_typ.GearRation2 = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.GearRation2 = aMachineData(0).GearRatio2
    '            If MN_TRQ01_typ.ForwardGearEfficiency = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.ForwardGearEfficiency = aMachineData(0).ForwardGearEfficiency
    '            If MN_TRQ01_typ.ReverseGearEfficiency = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.ReverseGearEfficiency = aMachineData(0).ReverseGearEfficiency
    '            If MN_TRQ01_typ.HiSpeedShaftInertia = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.HiSpeedShaftInertia = aMachineData(0).HiSpeedShaftInertia
    '            If MN_TRQ01_typ.MaxShaftSheaveLoad = "" Or MN_TRQ01_typ.bDirty Then MN_TRQ01_typ.MaxShaftSheaveLoad = aMachineData(0).MaxShaftSheaveLoad
    '        ElseIf ME_MRM01Car_typ.MachineModel <> "" And ME_COM01Car_typ.Speed <> 0 And (ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, True) <> "" Or GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE) And ME_MRM01Car_typ.RopingNew <> "" Then
    '            If Strings.Left(ME_MRM01Car_typ.MachineModel, 3) = GTE Then
    '                sSQL = "SELECT DISTINCT IIf([Default]='',0,[Default]), " & "SuperTable.MachineModel, SuperTable.[Machine Inertia], SuperTable.[Sheave Dia], SuperTable.[Std Susp Ld], SuperTable.[Worm Wheel], SuperTable.[Worm Thread], SuperTable.[Actual FGE], SuperTable.[Actual RGE], SuperTable.[Brake Drum/Disk], SuperTable.[HSS Inertia], SuperTable.Wrap " & "FROM SuperTable " & "GROUP BY IIf([Default]='',0,[Default]), SuperTable.MachineModel, SuperTable.[Machine Inertia], SuperTable.[Sheave Dia], SuperTable.[Std Susp Ld], SuperTable.[Worm Wheel], SuperTable.[Worm Thread], SuperTable.[Actual FGE], SuperTable.[Actual RGE], SuperTable.[Brake Drum/Disk], SuperTable.[HSS Inertia], SuperTable.Wrap, CStr([SuperTable].[Speed]), SuperTable.Roping " & "HAVING " & "SuperTable.MachineModel='" & ME_MRM01Car_typ.MachineModel & "' AND " & "SuperTable.Wrap=" & CStr(IIf(ME_MRM01Car_typ.Wrap = "Single Wrap", 1, 2)) & " AND " & "CStr([SuperTable].[Speed])='" & CStr(ME_COM01Car_typ.Speed) & "' AND " & "SuperTable.Roping=" & Strings.Left(ME_MRM01Car_typ.RopingNew, 1)
    '                WorkingRecordSet = New ADODB.Recordset
    '                WorkingRecordSet.Open(sSQL, ADOConnection)
    '            Else
    '                If ME_MRM01Car_typ.MachineVendor = "Westinghouse" And GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then
    '                    sSQL = "SELECT DISTINCT IIf([Default]='',0,[Default]), " & "MachineModel, [Machine Inertia], [Sheave Dia], [Std Susp Ld], [Worm Wheel], [Worm Thread], [Actual FGE], [Actual RGE], [Brake Drum/Disk], [HSS Inertia], HP, Wrap, Roping, Capacity " & "FROM SuperTable " & "WHERE " & "MachineModel='" & ME_MRM01Car_typ.MachineModel & "' AND " & (IIf(GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE, "SuperTable.Wrap=" & (IIf(ME_MRM01Car_typ.Wrap = "Single Wrap", 1, 2)) & " AND ", "")) & "CStr('' &  [SuperTable].[Speed])='" & CStr(ME_COM01Car_typ.Speed) & "' AND " & (IIf(GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE, "", "SuperTable.RPM=" & ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False) & " AND ")) & "SuperTable.Capacity=" & CStr(ME_COM01Car_typ.Capacity) & " AND " & "SuperTable.Roping=" & Strings.Left(ME_MRM01Car_typ.RopingNew, 1)
    '                Else
    '                    sSQL = "SELECT DISTINCT IIf([Default]='',0,[Default]), " & "MachineModel, [Machine Inertia], [Sheave Dia], [Std Susp Ld], [Worm Wheel], [Worm Thread], [Actual FGE], [Actual RGE], [Brake Drum/Disk], [HSS Inertia], HP, Wrap, Roping, Capacity " & "FROM SuperTable " & "WHERE " & "MachineModel='" & ME_MRM01Car_typ.MachineModel & "' AND " & (IIf(GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE, "SuperTable.Wrap=" & (IIf(ME_MRM01Car_typ.Wrap = "Single Wrap", 1, 2)) & " AND ", "")) & "CStr('' & [SuperTable].[Speed])='" & CStr(ME_COM01Car_typ.Speed) & "' AND SuperTable.[Sheave Dia]=" & CStr(Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia)) & " AND " & (IIf(GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE, "", "SuperTable.RPM=" & ExtractMotorRPM(MN_TRQ01_typ.NomimalMotorRpm, False) & " AND ")) & "SuperTable.Roping=" & Strings.Left(ME_MRM01Car_typ.RopingNew, 1)
    '                End If
    '                sOrderBy = " ORDER BY IIf([Default]='',0,[Default]) DESC;"
    '                WorkingRecordSet = New ADODB.Recordset
    '                WorkingRecordSet.Open(sSQL & sOrderBy, ADOConnection)
    '            End If
    '            If Not WorkingRecordSet.EOF Then
    '                Do While Not WorkingRecordSet.EOF
    '                    bCriteria = ME_MRM01Car_typ.BrakeType = "Disk" And WorkingRecordSet("Brake Drum/Disk").Value = 2
    '                    If Not bCriteria Then
    '                        If ME_MRM01Car_typ.BrakeType = "Drum" Then
    '                            If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then bCriteria = True
    '                            If Not bCriteria Then
    '                                bCriteria = ME_MRM01Car_typ.BrakeType = "Drum" And WorkingRecordSet("Brake Drum/Disk").Value = 1
    '                            End If
    '                        End If
    '                    End If
    '                    If bCriteria Then
    ...
    '                        If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '                            aMachineData(i).GearRatio2 = WorkingRecordSet("Worm Wheel").Value & ":" & WorkingRecordSet("Worm Thread").Value
    '                            aMachineData(i).GearRatio1 = ""
    '                            If WorkingRecordSet("Worm Thread").Value > 0 Then
    '                                aMachineData(i).GearRatio1 = CStr(Conversion.Val(Format(WorkingRecordSet("Worm Wheel").Value / WorkingRecordSet("Worm Thread").Value, "0.####")))
    '                            End If
    '                        Else
    ...
    '                            If WorkingRecordSet("Actual FGE").Value <> 0 Then
    '                                Select Case WorkingRecordSet("Actual FGE").Value
    '                                    Case 1 : aMachineData(i).ForwardGearEfficiency = CStr(0.65)
    '                                    Case 2 : aMachineData(i).ForwardGearEfficiency = CStr(0.74)
    '                                    Case 3 : aMachineData(i).ForwardGearEfficiency = CStr(0.78)
    '                                    Case Is >= 4 : aMachineData(i).ForwardGearEfficiency = CStr(0.8)
    '                                    Case Else : aMachineData(i).ForwardGearEfficiency = Format(WorkingRecordSet("Actual FGE").Value, "###0.0###")
    '                                End Select
    '                            Else
    '                                aMachineData(i).ForwardGearEfficiency = Format(WorkingRecordSet("Actual FGE").Value, "###0.0###")
    '                            End If
    '                        End If
    '                        If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then
    ...
    '                            If WorkingRecordSet("Actual RGE").Value <> 0 Then
    '                                Select Case WorkingRecordSet("Actual RGE").Value
    '                                    Case 1 : aMachineData(i).ReverseGearEfficiency = CStr(0.55)
    '                                    Case 2 : aMachineData(i).ReverseGearEfficiency = CStr(0.64)
    '                                    Case 3 : aMachineData(i).ReverseGearEfficiency = CStr(0.68)
    '                                    Case Is >= 4 : aMachineData(i).ReverseGearEfficiency = CStr(0.7)
    '                                    Case Else : aMachineData(i).ReverseGearEfficiency = Format(WorkingRecordSet("Actual RGE").Value, "###0.0###")
    '                                End Select
    '                            Else
    '                                aMachineData(i).ReverseGearEfficiency = CStr(Conversion.Val(aMachineData(i).ForwardGearEfficiency) - 0.1)
    '                            End If
    '                        End If
    '                        aMachineData(i).MachineSheaveDia = WorkingRecordSet("Sheave Dia").Value
    '                        aMachineData(i).MachineSheaveJ = ""
    '                        Select Case WorkingRecordSet("Machine Inertia").Value
    '                            Case 0
    '                                If WorkingRecordSet("Sheave Dia").Value <= 30 Then
    '                                    aMachineData(i).MachineSheaveJ = "17.00"
    '                                ElseIf WorkingRecordSet("Sheave Dia").Value > 36 Then
    '                                    aMachineData(i).MachineSheaveJ = "68.00"
    '                                Else
    '                                    aMachineData(i).MachineSheaveJ = "34.00"
    '                                End If
    '                            Case 1 : aMachineData(i).MachineSheaveJ = "17.00"
    '                            Case 2 : aMachineData(i).MachineSheaveJ = "34.00"
    '                            Case 3 : aMachineData(i).MachineSheaveJ = "68.00"
    '                            Case Else : aMachineData(i).MachineSheaveJ = Format(WorkingRecordSet("Machine Inertia").Value, "0.####")
    '                        End Select
    ...
    '                                Select Case WorkingRecordSet("HSS Inertia").Value
    '                                    Case 0
    '                                        Select Case WorkingRecordSet("Roping").Value
    '                                            Case 1
    '                                                If WorkingRecordSet("Capacity").Value <= 4500 Then
    '                                                    aMachineData(i).HiSpeedShaftInertia = "0.6405"
    '                                                ElseIf WorkingRecordSet("Capacity").Value > 6000 Then
    '                                                    aMachineData(i).HiSpeedShaftInertia = "3.234"
    '                                                Else
    '                                                    aMachineData(i).HiSpeedShaftInertia = "1.3251"
    '                                                End If
    '                                            Case 2
    '                                                If WorkingRecordSet("Capacity").Value <= 9000 Then
    '                                                    aMachineData(i).HiSpeedShaftInertia = "0.6405"
    '                                                ElseIf WorkingRecordSet("Capacity").Value > 12000 Then
    '                                                    aMachineData(i).HiSpeedShaftInertia = "3.234"
    '                                                Else
    '                                                    aMachineData(i).HiSpeedShaftInertia = "1.3251"
    '                                                End If
    ...
    '                                        End Select
    '                                    Case 1 : aMachineData(i).HiSpeedShaftInertia = "0.2571"
    '                                    Case 2 : aMachineData(i).HiSpeedShaftInertia = "0.6405"
    '                                    Case 3 : aMachineData(i).HiSpeedShaftInertia = "1.3251"
    '                                    Case 4 : aMachineData(i).HiSpeedShaftInertia = "3.234"
    '                                    Case Else : aMachineData(i).HiSpeedShaftInertia = WorkingRecordSet("HSS Inertia").Value
    '                                End Select
    '                                aMachineData(0).HiSpeedShaftInertia = CStr(Math.Round(CDbl(Conversion.Val(aMachineData(0).HiSpeedShaftInertia) + HSSInertiaAdder), 4))
    '                            End If
    '                        End If
    '                        aMachineData(i).MaxShaftSheaveLoad = WorkingRecordSet("Std Susp Ld").Value
    '                        i += 1
    '                    End If
    '                    WorkingRecordSet.MoveNext()
    '                Loop
    '                WorkingRecordSet.Close()
    '                WorkingRecordSet = Nothing
    '                If i = 0 Then MN_TRQ01_typ.bSuperTable = False
    '                Dependents.Set_DeflectorSheaveJ()
    '                If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then
    '                    aMachineData(0).GearRatio2 = "1:1"
    '                    aMachineData(0).GearRatio1 = "1"
    ...
    '                    End If
    '                End If
    '                If MN_TRQ01_typ.MachineSheaveJ = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.MachineSheaveJ = aMachineData(0).MachineSheaveJ
    '                If MN_TRQ01_typ.GearRation1 = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.GearRation1 = aMachineData(0).GearRatio1
    '                If MN_TRQ01_typ.GearRation2 = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.GearRation2 = aMachineData(0).GearRatio2
    '                If MN_TRQ01_typ.ForwardGearEfficiency = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.ForwardGearEfficiency = aMachineData(0).ForwardGearEfficiency
    '                If MN_TRQ01_typ.ReverseGearEfficiency = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.ReverseGearEfficiency = aMachineData(0).ReverseGearEfficiency
    '                If MN_TRQ01_typ.HiSpeedShaftInertia = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.HiSpeedShaftInertia = aMachineData(0).HiSpeedShaftInertia
    '                If MN_TRQ01_typ.MaxShaftSheaveLoad = "" Or MN_TRQ01_typ.bDirty Or Dependents.StatusChangeMachineToReplaceNew Then MN_TRQ01_typ.MaxShaftSheaveLoad = aMachineData(0).MaxShaftSheaveLoad
    '            Else
    '                MN_TRQ01_typ.bSuperTable = False
    '                WorkingRecordSet.Close()
    '                WorkingRecordSet = Nothing
    ...
    '        If Dependents.CarToCwtRopeDrop = SET_TO_ACTIVE Then
    '            If Math.Round(GONumbers(CurrentGOSelection).FileVersion, 1) < Math.Round(2013.2, 1) Then
    '                If Conversion.Val(MN_TRQ01_typ.RopeDrop) = 0 Then
    '                    MN_TRQ01_typ.RopeDrop = ME_MRM01Car_typ.CarToCwtRopeDrop.ToString
    '                End If
    '            Else
    '                MN_TRQ01_typ.RopeDrop = ME_MRM01Car_typ.CarToCwtRopeDrop.ToString
    '            End If
    ...
    '        Dim sSQL As String = "SELECT HatchEfficiency.HatchEff " & "FROM ShoeTypes INNER JOIN (Roping INNER JOIN (MRLocations INNER JOIN HatchEfficiency ON MRLocations.MRLocationID = " & "HatchEfficiency.MRLocation) ON Roping.RopingID = HatchEfficiency.Roping) ON ShoeTypes.ShoeID = HatchEfficiency.ShoeType " & "WHERE Roping.Roping='" & ME_MRM01Car_typ.RopingNew & "' AND ShoeTypes.ShoeType = '" & ME_CAR01Car_typ.GuidesCarType & "' AND MRLocations.MRLocation = '" & ME_MRM01Car_typ.MachineLocation & "';"
    '        WorkingRecordSet = New ADODB.Recordset
    '        WorkingRecordSet.Open(sSQL, ADOConnection)
    '        If Not WorkingRecordSet.EOF Then MN_TRQ01_typ.HatchEfficiency = WorkingRecordSet("HatchEff").Value.ToString
    '        WorkingRecordSet.Close()
    '        WorkingRecordSet = Nothing
    ...
    '                            iWrapAngleLogic = 1
    '                        Else
    '                            If Conversion.Val(ME_MRM01Car_typ.MachineSheaveDia) = HoistwaySheaveDiameter() Then
    '                                iWrapAngleLogic = 51
    '                            Else
    '                                iWrapAngleLogic = 52
    '                            End If
    ...
    '    End Function
    '    Public Function Get_CompUnitWeight() As Double
    ...
    '        WorkingRecordSet.Open(sSQL, ADOConnection)
    '        Do While Not WorkingRecordSet.EOF
    '            If WorkingRecordSet("CompName").Value.ToUpper() = (IIf(ME_PIT01Car_typ.CompensationType = "Cable", "Rope", ME_PIT01Car_typ.CompensationType)).ToUpper() Then
    '                If WorkingRecordSet("CompSize1").Value.ToString = ME_PIT01Car_typ.CableChainSize Then
    '                    result = WorkingRecordSet("CompSize2").Value
    '                    WorkingRecordSet.Close()
    '                    WorkingRecordSet = Nothing
    '                    Return result
    '                End If
    ...
    '        If ME_HstWay02Car_typ.HoistwaySheave = "New" Or ME_HstWay02Car_typ.HoistwaySheave = "Replace" Then
    '            Return Conversion.Val(ME_HstWay02Car_typ.HoistwaySheaveDia)
    '        Else
    '            Return Conversion.Val(MN_TRQ01_typ.DeflectorSheaveDiameter)
    '        End If
    ...
    '        Dim DebugFile As String = DebugPath & HoldUniqueActivity & "_" & GONumbers(CurrentGOSelection).Bank & GONumbers(CurrentGOSelection).Alt & _
    '                                  Strings.Left(GONumbers(CurrentGOSelection).Units, 2) & TORQUE_DEBUG_DETAIL_FILE_NAME
    ...
    '        sDebugInfoIn = sDebugInfoIn & "Torque Run for;" & MAIN_MDI_typ.GONumber & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Machine Room Location;" & ME_MRM01Car_typ.MachineLocation & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Capacity;" & CStr(ME_COM01Car_typ.Capacity) & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Car Weight Calculated;" & MN_TRQ01_typ.CarWeightCalculated & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "RiseFt;" & CStr(ME_COM01Car_typ.TravelFt) & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "RiseIn;" & CStr(ME_COM01Car_typ.TravelIn) & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Speed;" & CStr(ME_COM01Car_typ.Speed) & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Wrap;" & ME_MRM01Car_typ.Wrap & Environment.NewLine
    ...
    '        sDebugInfoIn = sDebugInfoIn & "Machine Sheave Diameter;" & ME_MRM01Car_typ.MachineSheaveDia & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Machine Sheave Inertia;" & MN_TRQ01_typ.MachineSheaveJ & Environment.NewLine
    '        If GONumbers(CurrentGOSelection).MachineType = GEARED_TYPE Then
    '            sDebugInfoIn = sDebugInfoIn & "Deflector Sheave Diameter;" & ME_HstWay02Car_typ.HoistwaySheaveDia & Environment.NewLine
    '            sDebugInfoIn = sDebugInfoIn & "Deflector Sheave Inertia;" & MN_TRQ01_typ.DeflectorSheaveJ & Environment.NewLine
    '        Else
    '            sDebugInfoIn = sDebugInfoIn & "Secondary Sheave Diameter;" & ME_HstWay02Car_typ.HoistwaySheaveDia & Environment.NewLine
    ...
    '        sDebugInfoIn = sDebugInfoIn & "Roping;" & ME_MRM01Car_typ.RopingNew & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Hoist Rope Diameter;" & ME_MRM01Car_typ.HoistCableSize & Environment.NewLine
    '        sDebugInfoIn = sDebugInfoIn & "Number of Ropes;" & CStr(ME_MRM01Car_typ.HoistCableQty) & Environment.NewLine
    ...
    '        sDebugInfoOut = sDebugInfoOut & "Comp Type; " & ME_PIT01Car_typ.CompensationType & Environment.NewLine
    '        sDebugInfoOut = sDebugInfoOut & "Number of Comps; " & CStr(ME_PIT01Car_typ.CableChainQty) & Environment.NewLine
    '        sDebugInfoOut = sDebugInfoOut & "Comp size; " & ME_PIT01Car_typ.CableChainSize & Environment.NewLine
    '        sDebugInfoOut = sDebugInfoOut & "Comp unit Weight; " & CStr(Get_CompUnitWeight()) & Environment.NewLine
    '        sDebugInfoOut = sDebugInfoOut & "Total Comp Weight; " & (IIf(Conversion.Val(MN_TRQ01_typ.CompensationWeight) = 0, CStr(Get_CompUnitWeight() * ME_PIT01Car_typ.CableChainQty), MN_TRQ01_typ.CompensationWeight)) & Environment.NewLine
    ...
    '        sDebugInfoOut = sDebugInfoOut & "Building Power Supply Voltage;" & CStr(ME_COM01Car_typ.PowerSupply) & Environment.NewLine
    '        If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then
    ...
    '            sDebugInfoOut = sDebugInfoOut & "Full Field Voltage;" & ME_MRM01Car_typ.VFF & Environment.NewLine
    '            sDebugInfoOut = sDebugInfoOut & "Full Field Current;" & ME_MRM01Car_typ.IFF & Environment.NewLine
    '            sDebugInfoOut = sDebugInfoOut & "Fan Voltage;" & ME_MRM01Car_typ.VFan & Environment.NewLine
    '            sDebugInfoOut = sDebugInfoOut & "Fan Currnent;" & ME_MRM01Car_typ.IFan & Environment.NewLine
    '        End If
    ...
    '        Else
    '            sDebugPowerData = sDebugPowerData & CStr(MN_TRQ01_typ.Required_KVA) & " KVA    @ " & CStr(ME_COM01Car_typ.PowerSupply) & " Building Voltage"
    '            s = Strings.FormatNumber(MN_TRQ01_typ.Required_KVA * 1000 / (ME_COM01Car_typ.PowerSupply * 3 ^ 0.5), 2, TriState.True) & " Amps"
    ...
    '            DebugFile = DebugPath & HoldUniqueActivity & "_" & GONumbers(CurrentGOSelection).Bank & GONumbers(CurrentGOSelection).Alt & _
    '                        Strings.Left(GONumbers(CurrentGOSelection).Units, 2) & TORQUE_DEBUG_DETAIL_FILE_NAME
    '            FileSystem.FileOpen(FileNumber, DebugFile, OpenMode.Output)
    '            FileSystem.PrintLine(FileNumber, sDebugInfo)
    '            FileSystem.FileClose(FileNumber)
    ...
    '        sDebugPowerData = ""
    '        sDebugPowerData = sDebugPowerData & "Drive Selection " & Strings.Chr(9) & Strings.Chr(9) & Strings.Chr(9) & "Drive Type = " & Strings.Chr(34).ToString() & ME_MRM01Car_typ.ControllerType & Strings.Chr(34).ToString() & Environment.NewLine
    ...
    '    End Sub
    '    Private Sub Select_RopeGripperModel()
    '        Dim HangingLoad As Single = 0
    '        Dim CarAndCwtWeight As Single = 0
    '        Dim OutToOutOfRopeDimension As Single = 0
    '        Dim UseRopeGripperModel As String = ""

    '        Erase RopeGripperCriteria
    '        ReDim RopeGripperCriteria(8)
    '        For iIndex As Integer = RopeGripperCriteria.GetLowerBound(0) To RopeGripperCriteria.GetUpperBound(0)
    '            RopeGripperCriteria(iIndex).OutToOutMax = -999
    '            RopeGripperCriteria(iIndex).SpeedMax = -999
    '            RopeGripperCriteria(iIndex).CapacityMax = -999
    '            RopeGripperCriteria(iIndex).HangingLoadMax = -999
    '            RopeGripperCriteria(iIndex).CapacityMin = -999
    '            RopeGripperCriteria(iIndex).CarAndCwtWeightMin = -999
    '        Next iIndex
    '        If ME_MRM01Car_typ.RopingNew = "1:1" Then
    '            RopeGripperCriteria(HW620).OutToOutMax = 4.875
    '            RopeGripperCriteria(HW620).SpeedMax = 350
    '            RopeGripperCriteria(HW620).CapacityMax = 2500
    '            RopeGripperCriteria(HW620).HangingLoadMax = 11500
    '            RopeGripperCriteria(HW620).CapacityMin = 600
    '            RopeGripperCriteria(HW620).CarAndCwtWeightMin = 2280

    '            RopeGripperCriteria(HW622).OutToOutMax = 6
    '            RopeGripperCriteria(HW622).SpeedMax = 600
    '            RopeGripperCriteria(HW622).CapacityMax = 5000
    '            RopeGripperCriteria(HW622).HangingLoadMax = 18600
    '            RopeGripperCriteria(HW622).CapacityMin = 1500
    '            RopeGripperCriteria(HW622).CarAndCwtWeightMin = 6000

    '            RopeGripperCriteria(HW624).OutToOutMax = 10
    '            RopeGripperCriteria(HW624).SpeedMax = 1200
    '            RopeGripperCriteria(HW624).CapacityMax = 5000
    '            RopeGripperCriteria(HW624).HangingLoadMax = 18600
    '            RopeGripperCriteria(HW624).CapacityMin = 1500
    '            RopeGripperCriteria(HW624).CarAndCwtWeightMin = 6000

    '            RopeGripperCriteria(HW625).OutToOutMax = 11.5
    '            RopeGripperCriteria(HW625).SpeedMax = 1200
    '            RopeGripperCriteria(HW625).CapacityMax = 5000
    '            RopeGripperCriteria(HW625).HangingLoadMax = 18600
    '            RopeGripperCriteria(HW625).CapacityMin = 1500
    '            RopeGripperCriteria(HW625).CarAndCwtWeightMin = 6000

    '            RopeGripperCriteria(HW626).OutToOutMax = 10
    '            RopeGripperCriteria(HW626).SpeedMax = 1200
    '            RopeGripperCriteria(HW626).CapacityMax = 10000
    '            RopeGripperCriteria(HW626).HangingLoadMax = 38000
    '            RopeGripperCriteria(HW626).CapacityMin = 2500
    '            RopeGripperCriteria(HW626).CarAndCwtWeightMin = 8000
    '        Else
    '            RopeGripperCriteria(HW620).OutToOutMax = 4.875
    '            RopeGripperCriteria(HW620).SpeedMax = 250
    '            RopeGripperCriteria(HW620).CapacityMax = 5000
    '            RopeGripperCriteria(HW620).HangingLoadMax = 23000
    '            RopeGripperCriteria(HW620).CapacityMin = 1500
    '            RopeGripperCriteria(HW620).CarAndCwtWeightMin = 6000

    '            RopeGripperCriteria(HW622).OutToOutMax = 6
    '            RopeGripperCriteria(HW622).SpeedMax = 400
    '            RopeGripperCriteria(HW622).CapacityMax = 10000
    '            RopeGripperCriteria(HW622).HangingLoadMax = 38000
    '            RopeGripperCriteria(HW622).CapacityMin = 2500
    '            RopeGripperCriteria(HW622).CarAndCwtWeightMin = 8000

    '            RopeGripperCriteria(HW624).OutToOutMax = 10
    '            RopeGripperCriteria(HW624).SpeedMax = 800
    '            RopeGripperCriteria(HW624).CapacityMax = 10000
    '            RopeGripperCriteria(HW624).HangingLoadMax = 38000
    '            RopeGripperCriteria(HW624).CapacityMin = 2500
    '            RopeGripperCriteria(HW624).CarAndCwtWeightMin = 8000

    '            RopeGripperCriteria(HW625).OutToOutMax = 11.5
    '            RopeGripperCriteria(HW625).SpeedMax = 800
    '            RopeGripperCriteria(HW625).CapacityMax = 10000
    '            RopeGripperCriteria(HW625).HangingLoadMax = 38000
    '            RopeGripperCriteria(HW625).CapacityMin = 2500
    '            RopeGripperCriteria(HW625).CarAndCwtWeightMin = 8000

    '            RopeGripperCriteria(HW626).OutToOutMax = 10
    '            RopeGripperCriteria(HW626).SpeedMax = 800
    '            RopeGripperCriteria(HW626).CapacityMax = 20000
    '            RopeGripperCriteria(HW626).HangingLoadMax = 76000
    '            RopeGripperCriteria(HW626).CapacityMin = 5000
    '            RopeGripperCriteria(HW626).CarAndCwtWeightMin = 16000
    '        End If
    '        If ME_MRM01Car_typ.RopeGripper = "New" Or ME_MRM01Car_typ.RopeGripper = "Replace" Then
    '            UseRopeGripperModel = "Invalid"
    '            If String.IsNullOrEmpty(MN_TRQ01_typ.RopeGripperModel) Or MN_TRQ01_typ.RopeGripperModel = "Invalid" Then
    '                HangingLoad = T1s + T2s
    '                CarAndCwtWeight = Conversion.Val(MN_TRQ01_typ.CarWeightCalculated) + dCWT
    '                OutToOutOfRopeDimension = Conversion.Val(Strings.Mid(ME_MRM01Car_typ.OutToOutOfRopeDimension, 3))
    '                For iIndex As Integer = 0 To 4
    '                    Select Case iIndex
    '                        Case 0
    '                            UseRopeGripperModel = "HW620"
    '                        Case 1
    '                            UseRopeGripperModel = "HW622"
    '                        Case 2
    '                            UseRopeGripperModel = "HW624"
    '                        Case 3
    '                            UseRopeGripperModel = "HW625"
    '                        Case 4
    '                            UseRopeGripperModel = "HW626"
    '                        Case Else
    '                    End Select
    '                    If OutToOutOfRopeDimension <= RopeGripperCriteria(iIndex).OutToOutMax And ME_COM01Car_typ.Speed <= RopeGripperCriteria(iIndex).SpeedMax And _
    '                       ME_COM01Car_typ.Capacity >= RopeGripperCriteria(iIndex).CapacityMin And ME_COM01Car_typ.Capacity <= RopeGripperCriteria(iIndex).CapacityMax And _
    '                       HangingLoad <= RopeGripperCriteria(iIndex).HangingLoadMax And CarAndCwtWeight >= RopeGripperCriteria(iIndex).CarAndCwtWeightMin Then
    '                        MN_TRQ01_typ.RopeGripperModel = UseRopeGripperModel
    '                        Exit For
    '                    End If
    '                Next iIndex
    '                If MN_TRQ01_typ.RopeGripperModel = "" Then
    '                    MN_TRQ01_typ.RopeGripperModel = "Invalid"
    '                End If
    '            End If
    '        Else
    '            MN_TRQ01_typ.RopeGripperModel = String.Empty
    '        End If
    ...
    '        Dim result As Double = 0
    '        Dim KVAFreeFileNumber As Integer
    '        Dim KVADebugFile As String = ""
    ...
    '        Dim InverterEfficiency As Double = IIf(ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR, 0.95, 0.91)
    '        Dim EP As Double = Conversion.Val(CStr(ME_COM01Car_typ.PowerSupply))
    '        Dim PA As Double = IIf(GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE And ME_MRM01Car_typ.ControllerType = DRIVETYPE_PF1, 3, 0)
    ...
    '        Dim KWRunningFLU As Double = (Math.Sqrt(3) * EP * dIrflu * (IIf(ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR, COS_Theta, PF1))) / 1000
    '        Dim A As Double = 7.2
    ...
    '        If ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR Then
    '            Select Case MN_TRQ01_typ.NomimalMotorRpm
    '                Case "700-1000"
    ...
    '        If Math.Round(MN_TRQ01_typ.Required_KVA, 2) = Math.Round(-999.99, 2) Or Dependents.StatusChangeHoistMotorRpm Or Dependents.StatusChangePowerSupply Or
    '           Dependents.StatusChangeControllerType Or (String.IsNullOrEmpty(MN_TRQ01_typ.DriveModel) And String.IsNullOrEmpty(MN_TRQ01_typ.DriveModelDefault)) Or
    '           bMotorDependentChange Or Dependents.StatusChangeCarWeightAvailable Then
    '            Select Case EP
    '                Case 208, 240
    ...
    '        End If
    '        If TestVersion Then
    '            KVAFreeFileNumber = FileSystem.FreeFile()
    '            KVADebugFile = DebugPath & HoldUniqueActivity & "_" & GONumbers(CurrentGOSelection).Bank & GONumbers(CurrentGOSelection).Alt & _
    '                           Strings.Left(GONumbers(CurrentGOSelection).Units, 2) & "KVA_DEBUG.TXT"
    '            If FileSystem.Dir(KVADebugFile, FileAttribute.Normal) <> "" Then File.Delete(KVADebugFile)
    '            FileSystem.FileOpen(KVAFreeFileNumber, KVADebugFile, OpenMode.Output)
    '            TQDebugResults(KVAFreeFileNumber, "KW dc", CStr(ConverterKW))
    '            TQDebugResults(KVAFreeFileNumber, "I", CStr(ConverterCurrent))
    '            TQDebugResults(KVAFreeFileNumber, "I line", CStr(LineCurrentAC))
    '            TQDebugResults(KVAFreeFileNumber, "I Running FLU", CStr(TmpIRFLU))
    '            TQDebugResults(KVAFreeFileNumber, "KW Running FLU", CStr(KWRunningFLU))
    '            TQDebugResults(KVAFreeFileNumber, "KVA d", CStr(MN_TRQ01_typ.Calculated_KVA))
    '            TQDebugResults(KVAFreeFileNumber, "KVA Rating", CStr(result))
    '            FileSystem.PrintLine(KVAFreeFileNumber, "-------------- Values used in Calcs -------------------")
    '            TQDebugResults(KVAFreeFileNumber, "TRFLU", CStr(dFullTork))
    '            TQDebugResults(KVAFreeFileNumber, "RPM", CStr(dRPM))
    '            TQDebugResults(KVAFreeFileNumber, "e m", CStr(MotorEfficiency))
    '            TQDebugResults(KVAFreeFileNumber, "e i", CStr(InverterEfficiency))
    '            TQDebugResults(KVAFreeFileNumber, "V line", CStr(VLine))
    '            If ME_MRM01Car_typ.ControllerType = DRIVETYPE_BR Then
    '                TQDebugResults(KVAFreeFileNumber, "COS Theta", CStr(COS_Theta))
    '            Else
    '                TQDebugResults(KVAFreeFileNumber, "pf1", CStr(PF1))
    '            End If
    '            TQDebugResults(KVAFreeFileNumber, "I t1", CStr(TransformerCurrent))
    '            TQDebugResults(KVAFreeFileNumber, "EP", CStr(EP))
    '            TQDebugResults(KVAFreeFileNumber, "PA", CStr(PA))
    '            FileSystem.FileClose(KVAFreeFileNumber)
    '        End If
    '        Return result

    '    End Function
    '    Public Function GetFMM200Value(ByRef FieldName As String) As Object
    '        Dim SpeedInMeters As Single = ME_COM01Car_typ.Speed / 200
    '        Dim UseCapacity As Integer = ME_COM01Car_typ.Capacity
    '        Dim TotalTravelInMeters As Integer = Math.Round((Feet_to_Inch(ME_COM01Car_typ.TravelFt) + ME_COM01Car_typ.TravelIn) * 0.0254)
    '        Dim sSQL As String = ""
    ...
    '            WorkingRecordSet = New ADODB.Recordset
    '            WorkingRecordSet.Open(sSQL, ADOConnection)
    '            If Not Convert.IsDBNull(WorkingRecordSet(FieldName).Value) Then
    '                ReturnValue = WorkingRecordSet(FieldName).Value
    '            End If
    ...
    '        sSQL = "SELECT * FROM [Lookup Defaults] WHERE " & "[Lookup Defaults].[Machine Vendor]='" & ME_MRM01Car_typ.MachineVendor & "' AND " &
    '               "[Lookup Defaults].[Machine Model] = '" & ME_MRM01Car_typ.MachineModel & "' AND " &
    '               "[Lookup Defaults].Speed=" & ME_COM01Car_typ.Speed.ToString & " AND " &
    '               "[Lookup Defaults].Capacity=" & ME_COM01Car_typ.Capacity.ToString & " AND " &
    '               "[Lookup Defaults].Roping='" & ME_MRM01Car_typ.RopingNew & "'"
    '        WorkingRecordSet = New ADODB.Recordset
    '        WorkingRecordSet.Open(sSQL, ADOConnection)
    '        If Not WorkingRecordSet.EOF Then
    '            ReturnVal = WorkingRecordSet(WhichVar).Value.ToString
    '        End If
    '        WorkingRecordSet.Close()
    '        WorkingRecordSet = Nothing
    '        Return ReturnVal
    ...
    '                            sRound2Next5 = "50"
    '                        ElseIf CInt(sRound2Next5) <= 60 And ME_MRM02Car_typ.Mounting = "Foot-Mounted" Then
    '                            sRound2Next5 = "60"
    '                        Else
    '                            sRound2Next5 = "75"
    '                        End If
    ...
    '        If GONumbers(CurrentGOSelection).MachineType = GEARLESS_TYPE Then
    '            If ME_MRM01Car_typ.ControllerType = DRIVETYPE_SCRDC Or ME_MRM01Car_typ.ControllerType = DRIVETYPE_QUATTRO Then
    '                If Conversion.Val(ME_MRM01Car_typ.IFF) = 0 Or _
    '                    Conversion.Val(ME_MRM01Car_typ.VFF) = 0 Or _
    ...
    '        Dim CarAtBottom As Double = dCarWeight + dCapacity + dRWT * (iRoping * dRise + dXRope / 2) + dCompWt * (dXCompChain / 2) + dCtrlCableWt * (dXtraCCable / 2) + dW1 / 2
    '        Dim CarAtTop As Double = dCarWeight + dCapacity + dRWT * (dXRope / 2) + dCompWt * (dRise + dXCompChain / 2) + dCtrlCableWt * (dRise + dXtraCCable) / 2 + dW1 / 2
    ...
    Public Function OtherOther(Optional ByVal FindSheaveDiameter As Boolean = True) As Boolean
        ...
        Else
            ...
            If Not ReturnVal Then
                UseRPM = ExtractMotorRPM(CurDataset)
    ...
    End Function
    Public Function ExtractMotorRPM(ByRef UseCurDataset As DataSet) As String
        Dim ReturnVal As String = String.Empty

        Select Case Conversion.Val(GetValue(UseCurDataset.Tables(TABLENAME_GENERALINFO), "HoistMotorRpm_txt"))
            Case Is <= 850
                ReturnVal = 850
            Case Is <= 1150
                ReturnVal = 1150
